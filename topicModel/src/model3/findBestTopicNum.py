...
if __name__=='__main__':
  ...
  df_less_all['toks_pdf2txt'] = df_less_all.toks_pdf2txt.apply(lambda x: [] if not len(x) else x)
  df_less_all['toks_metada'] = df_less_all.toks_metada.apply(lambda x: [] if not len(x) else x)
  ...
  # The function
  coherence_scores = find_topics(num_topics, corpus, texts=df_less_all.concat_toks.tolist(), dictionary=dict_restricted)
