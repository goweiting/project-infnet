...
if __name__ == "__main__":
  ...
  tmdblp.minimum_probability = 0.001 # 0.001 instad because larger number of topics
  ...
  ground_truth_adj_mat = np.load(os.path.join(
      DATA_DIR, 'mat', 'infnet6yrs-adj-mat.order6yr.pkl'))

  # Combine all tokens:
  topicnetref6yr = infnet6yr.join(
      df_pubmapping.set_index('id'), how='left', on='id')
  topicnetref6yr['toks'] = topicnetref6yr['pub_ids'].apply(
      lambda a: gen_toks(a, lookup_combined_toks))
  topicnetref6yr['tm_probs'] = topicnetref6yr['toks'].apply(
  ...
  cosim = compare_researchers(
      topicnetref6yr.tm_probs.tolist(), tmdblp.num_topics)
  ...
  plt.tight_layout()
  fig.savefig('IMG/topicnetref6yr_thresholding.png',
